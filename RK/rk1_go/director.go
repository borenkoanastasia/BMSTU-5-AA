...
var BIG_MUTEX sync.Mutex
var SYSTEM_MUTEX sync.Mutex
var SYSTEM_SEMAFORE[]sync.Mutex

func director(system_parameters *system_parameters_t){
	SYSTEM_SEMAFORE = make([]sync.Mutex, system_parameters.builders_count) //по семафору на каменщика +2 на диспетчера
