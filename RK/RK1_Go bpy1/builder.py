...
def generate_new_borders(work_res, weight_borders):
    ...
    for i in range(len(work_res)):
        ...
        borders.append(cur)
    res = []
    print(borders, weight_borders)
    for i in range(len(borders)-1):
        res.append([int(borders[i]),int(borders[i+1])])
    return res

def get_min_cur_line(bricks_matrix):
    for i in range(len(bricks_matrix)):
        ended_line = True
        for j in range(len(bricks_matrix)):
            if (bricks_matrix[i][j]) == 0:
                ended_line = False
                break
        if (ended_line == False):
            return i  
...
def change_dispetcher_borders(bricks_matrix, w):
    res = [0,w]
    line_id = get_min_cur_line(bricks_matrix)
    if (line_id == -1):
        return [-1, -1]
    print("line id",line_id)
    for i in range(len(bricks_matrix[line_id])):
        res[0] = i 
        if (bricks_matrix[line_id][i]) == 0:
            break
    for i in range(len(bricks_matrix[line_id]), 0):
        res[1] = i
        if (bricks_matrix[line_id][i]) == 0:
            break
    print(res)
