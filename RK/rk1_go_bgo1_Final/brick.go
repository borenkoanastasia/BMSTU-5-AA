...
var DISPETCHER_BORDERS_BUF []int
var DISPETCHER_RES_BUF [][]int
...
func make_random_string(lim len_limits_t)string{
	...
	var string_len = lim.min  
	if (lim.max - lim.min > 0){
		string_len+= rand.Intn(lim.max)
	}
	var cur_str string = ""
...
func make_builders_weight(builders_count int, weight int)[]int{
	var builders_weight = DISPETCHER_BORDERS_BUF//make([]int, builders_count + 1)
...
func make_builders_borders(builders_count int, weight int)[][]int{
    var builders_borders [][]int = DISPETCHER_RES_BUF//make([][]int, builders_count)
	//builders_borders
...
func make_builders_wait_status(builders_count int)[]bool{
	var res = make([]bool, builders_count)
	for i:=0;i<builders_count;i++{
		res[i] = false
	}
	return res
}
...
func init_system_parameters(w int, h int, bc int, len_lim len_limits_t, dispetcher_time int, builder_time int)system_parameters_t{
	...
	DISPETCHER_BORDERS_BUF = make([]int, bc+1)
	DISPETCHER_RES_BUF = make([][]int, bc)
	for i:=0;i<bc;i++{
		DISPETCHER_RES_BUF[i] = make([]int, 2)
	}
	...
	system_parameters.builders_wait_status = make_builders_wait_status(bc)
