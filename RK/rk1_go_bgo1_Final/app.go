...
var PINK = []float64{1, 0.90, 0.90}

var COLORS = [][]float64{RED, YELLOW, GREEN, BLUE, PINK}

func get_color(builder_name int)[]float64{
//	builder_name += len(COLORS)
	return COLORS[builder_name%len(COLORS)]
}
	...
	for i:=0;i<len((*BRICKS_MATRIX));i++{
		for j:=0;j<len((*BRICKS_MATRIX)[0]);j++{
			if ((*BRICKS_MATRIX)[i][j].fixed == true){
				...
				var color []float64 = get_color((*BRICKS_MATRIX)[i][j].builder_name)
