...
func (colony *Colony) createAnt(position int) *Ant {
	ant := new(Ant)
	ant.colony = colony
	ant.visited = make([][]float64, len(colony.graph))

	for i := 0; i < len(colony.graph); i++ {
		ant.visited[i] = make([]float64, len(colony.graph))
...
func CreateColony(graph [][]float64, days int) *Colony {
	colony := new(Colony)
	colony.graph = graph
	colony.days = days
...
func GoAnts(colony *Colony) []float64 {
	shortest := make([]float64, len(colony.graph))
